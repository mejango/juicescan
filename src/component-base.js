...
export var COMPONENT_SPECS = {
  ...
  'project-payer': { file: 'discover.js (renderExtrasSection / buildProjectPayerDeployArgs / fetchProjectPayerRows)', fn: 'JBProjectPayerDeployer.deployProjectPayer(uint256 defaultProjectId, address defaultBeneficiary, string defaultMemo, bytes defaultMetadata, bool defaultAddToBalance, address owner) returns (address projectPayer)', desc: "Deploys a fresh JBProjectPayer forwarding address for a project on each selected chain. The deploy is permissionless and goes through JBProjectPayerDeployer; the UI defaults to pay mode (defaultAddToBalance=false), zero beneficiary (meaning the original payer receives the project's tokens), empty memo, 0x metadata, and zero address admin (an immutable payer). Checking Editable reveals the admin field and defaults it to the connected wallet. Gotchas: defaultBeneficiary and owner can differ by chain via the per-chain address-control pattern; a nonzero owner/admin can later change the payer defaults but does NOT receive funds or control the project, while a zero owner can never edit them. The receive() path only handles native ETH and forwards through the terminal using the stored defaults; ERC-20s such as USDC are supported by calling the payer's pay/addToBalanceOf functions after token approval, not by direct token transfer. Metadata must be even-length hex bytes. The list under the form is indexed from Bendystraw's projectPayers table, ordered by totalFacilitatedUsd descending, and only appears after the deployer event and terminal Pay/AddToBalance events have been indexed." },
